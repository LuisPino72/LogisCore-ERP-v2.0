 ...
 * - Errores esta semana
 * - Uptime del sistema
 * - Tenants, usuarios y suscripciones totales
 */
...
export function SystemMetricsPanel({ metrics, isLoading, onRefresh }: SystemMetricsPanelProps) {
  useEffect(() => {
    onRefresh();
  }, [onRefresh]);
  ...
  const statCards = [
    ...
    { 
      ...
      color: metrics?.errorsThisWeek && metrics.errorsThisWeek > 0 ? "text-state-error" : "text-state-success",
      bgColor: metrics?.errorsThisWeek && metrics.errorsThisWeek > 0 ? "bg-red-50" : "bg-green-50",
      borderColor: metrics?.errorsThisWeek && metrics.errorsThisWeek > 0 ? "border-red-200" : "border-green-200"
    },
    { 
      label: "Uptime", 
  ...
  const coreMetrics = [
    ...
    { label: "Total Usuarios", value: metrics?.totalUsers ?? 0 },
    { label: "Suscripciones Activas", value: metrics?.activeSubscriptions ?? 0 }
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {coreMetrics.map((metric, index) => (
          <div 
            key={metric.label} 
            className="card hover:border-brand-200 transition-all duration-300"
            style={{ animationDelay: `${(index + 4) * 0.1}s` }}
          >
            <div className="card-body flex items-center justify-between">
              <div>
                <p className="text-sm text-content-secondary">{metric.label}</p>
                <p className="text-2xl font-bold text-content-primary mt-1">{metric.value}</p>
              </div>
      ...
      <div className="text-center text-xs text-content-tertiary pt-4">
        <p>LogisCore ERP v2.0 • Panel de Métricas • Solo visible para administración</p>
      </div>
    </div>
  );
}