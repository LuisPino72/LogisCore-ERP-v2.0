  ...
  async listReceivings(tenantId: string): Promise<Receiving[]> {
    ...
    return records.map((r) => {
      ...
      const item: Receiving = {
        ...
        status: rr.status as unknown as Receiving['status'],
        items: (rr.items as unknown as Receiving['items']) ?? [],
        receivedItems: (rr.receivedItems as unknown as Receiving['receivedItems']) ?? [],
